...
int main() {
  ...
  int n, k, a, b, c;
  cin >> n >> k >> a >> b >> c;
  function<int(int, int, int)> cal = [&](int x, int y, int kk) {
    return x * n + y + kk * n * n;
  };
  int S = cal(n - 1, n - 1, k) + 1;
  int T = S + 1;
  Primal_Dual<long long, long long> G(T + 1);
  G.addEdge(S, cal(0, 0, k), 1, 0);
  for (int i = 0; i <= k; ++i) {
    G.addEdge(cal(n - 1, n - 1, i), T, 1, 0);
  }
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      int x;
      ...
      if (x == 0) {
        for (int p = 1; p <= k; ++p) {
          if (i - 1 >= 0) {
            G.addEdge(cal(i, j, p), cal(i - 1, j, p - 1), 1, b);
          }
          if (j - 1 >= 0) {
            G.addEdge(cal(i, j, p), cal(i, j - 1, p - 1), 1, b);
          }
          if (i + 1 < n) {
            G.addEdge(cal(i, j, p), cal(i + 1, j, p - 1), 1, 0);
          }
          if (j + 1 < n) {
            G.addEdge(cal(i, j, p), cal(i, j + 1, p - 1), 1, 0);
          }
        }
        G.addEdge(cal(i, j, 0), cal(i, j, k), 1, a + c);
      } else {
        for (int p = 0; p < k; ++p) {
          G.addEdge(cal(i, j, p), cal(i, j, k), 1, a);
        }
        if (i - 1 >= 0) {
          G.addEdge(cal(i, j, k), cal(i - 1, j, k - 1), 1, b);
        }
        if (j - 1 >= 0) {
          G.addEdge(cal(i, j, k), cal(i, j - 1, k - 1), 1, b);
        }
        if (i + 1 < n) {
          G.addEdge(cal(i, j, k), cal(i + 1, j, k - 1), 1, 0);
        }
        if (j + 1 < n) {
          G.addEdge(cal(i, j, k), cal(i, j + 1, k - 1), 1, 0);
        }
      }
    }
  }
