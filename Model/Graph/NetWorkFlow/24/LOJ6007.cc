...
int main() {
  ...
  int S = n * m, T = n * m + 1;
  function<int(int, int)> cal = [&](int x, int y) { return x * n + y; };
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      if ((i + j) % 2 == 0) {
        G.addEdge(S, cal(i, j), mp[i][j]);
        if (i - 1 >= 0) {
          G.addEdge(cal(i, j), cal(i - 1, j), inf);
        }
        if (i + 1 < m) {
          G.addEdge(cal(i, j), cal(i + 1, j), inf);
        }
        if (j - 1 >= 0) {
          G.addEdge(cal(i, j), cal(i, j - 1), inf);
        }
        if (j + 1 < n) {
          G.addEdge(cal(i, j), cal(i, j + 1), inf);
        }
      } else {
        G.addEdge(cal(i, j), T, mp[i][j]);
      }
    }
  }
