...
int main() {
  ...
  function<int(int)> clear = [&](int i) { return i * 2; };
  function<int(int)> dirty = [&](int i) { return i * 2 + 1; };
  int Source = n * 2, Sink = n * 2 + 1;
  for (int i = 0; i < n; ++i) {
    int p;
    cin >> p;
    G.addEdge(Source, clear(i), inf, P);
    if (i - 1 >= 0)
      G.addEdge(dirty(i - 1), dirty(i), inf, 0);
    G.addEdge(Source, dirty(i), p, 0);
    G.addEdge(clear(i), Sink, p, 0);
    if (i + M < n) {
      G.addEdge(dirty(i), clear(i + M), inf, F);
    }
    if (i + N < n) {
      G.addEdge(dirty(i), clear(i + N), inf, S);
    }
  }
  auto [_, price] = G.solve(Source, Sink);
  cout << price << "\n";
}