...
auto solve() {
  ...
  for (int i = 0; i < n; ++i) {
    int dis = (a[i] < i ? a[i] + n - i : a[i] - i);
    Q.emplace(dis, i);
  }
  lazy_segtree<node, op, e, F, mapping, composition, id> lef(n * 2), rig(n * 2);
  for (int i = 0; i < n * 2; ++i) {
    lef.set(i, {0, 1});
    rig.set(i, {0, 1});
  }
  ...
  while (!Q.empty()) {
    ...
    Q.pop();
    // cout << i+1 << " " << a[i]+1 << " ";
    // cout << d << " " << segnum << " " << lef.prod(0, i).sum << " "
    //  << rig.prod(i + d, n * 2).sum << "\n";
    ans[a[i]] = d - (segnum - lef.prod(0, i).sum - rig.prod(i + d, n * 2).sum);
    assert(ans[a[i]] >= 0);
    segnum++;
    lef.set(i, {1, 1});
    rig.set(i + d, {1, 1});
