  ...
  while (q--) {
    ...
    if (op == 1) {
      ...
      int lef = [&]() {
        ...
        while (l <= r) {
          int mid = (l + r) / 2;
          if (seg.prod(0, mid + 1).sum >= global_s) {
            ans = min(ans, mid);
            r = mid - 1;
          } else {
            l = mid + 1;
      ...
      int rig = [&]() {
        ...
        while (l <= r) {
          int mid = (l + r) / 2;
          if (seg.prod(mid, n).sum >= global_s) {
            ans = max(ans, mid);
            l = mid + 1;
          } else {
            r = mid - 1;
      ...
      // cout << lef << " " << rig << "\n";
      auto it = one.lower_bound(lef);
      auto itr = one.upper_bound(rig);
      if (seg.prod(0, lef + 1).sum == global_s ||
          seg.prod(rig, n).sum == global_s || it != one.end() ||
          itr != one.begin()) {
        cout << "YES\n";
      } else {
        cout << "NO\n";
