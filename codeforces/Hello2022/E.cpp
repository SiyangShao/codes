...
void solve() {
	...
	sort(avg.rbegin(), avg.rend(),
		 [&](PPI A, PPI B) { return comp(A.first, B.first); });
	vector<int> pos(m);
	for (int i = 0; i < m; i++)
		pos[avg[i].second] = i;
	...
	for (int i = 0; i < m; i++) {
		int id = pos[i];
		PII cur = {accumulate(g[i].begin(), g[i].end(), 0LL), g[i].size() - 1};
		for (int j = 0; j < g[i].size(); j++) {
			...
			if (R > id)
				R--;
			int tr = 1;
			if (comp({1ll * a[R], 1}, cur) == 1)
				tr = 0;
			if (min(R, id) - 1 >= 0 && assign_to_this[min(R, id) - 1] != 0)
				tr = 0;
			if (assign_to_this[max(R, id)] != assign_to_this.back())
				tr = 0;
			if (R < id &&
				(R ? assign_to_next[R - 1] : 0) != assign_to_next[id - 1])
				tr = 0;
			if (R > id && assign_to_prev[R] != assign_to_prev[id])
				tr = 0;
			cout << tr;
			cur.first += g[i][j];
		}
