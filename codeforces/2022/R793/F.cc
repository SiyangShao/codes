...
using ll = modint1000000007;
auto solve() {
  int n, q;
  cin >> n >> q;
  vector<ll> a(n + 1), b(n + 1);
  vector<long long> ori(n + 1);
  vector<ll> dif(n + 1), suf(n + 1), f(n + 1);
  vector<int> id(n);
  ...
  sort(id.begin(), id.end(), [&](auto x, auto y) { return ori[x] > ori[y]; });
  fenwick_tree<ll> c(n + 1), d(n + 1);
  for (int i = 1; i <= n; ++i) {
    suf[i] = suf[i - 1] + dif[i] * b[i];
  }
  for (auto i : id) {
    f[i] = (a[i] - 2 * c.sum(0, i + 1)) * b[i] + d.sum(0, i + 1) - suf[i - 1];
    c.add(i, dif[i]);
    d.add(i, dif[i] * b[i] * 2);
  }
  while (q--) {
