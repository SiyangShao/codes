...
node e() { return {-inf, inf}; }
using F = long long;
node mapping(F f, node x) { return {x.mx + f, x.mi + f}; }
F composition(F f, F g) { return F{f + g}; } // f(g())
F id() { return F{0}; }
auto sol(const auto &a) {
  int n = (int)a.size();
  ...
  ll ans = a[id.back()] - a[id.front()];
  ll cur = a[id.front()];
  int l = id.front(), r = id.front();
  for (auto i : id) {
    if (a[i] != cur) {
      if (l == 0 && r == n - 1) {
        return ans;
      }
      auto lef = seg.prod(0, l);
      auto rig = seg.prod(r + 1, n);
      auto x = seg.prod(l, r + 1);
      auto curAns = max(x.mx - x.mi, max(lef.mx, rig.mx) - min(lef.mi, rig.mi));
      ans = min(ans, curAns);
    }
    l = min(l, i);
    r = max(r, i);
