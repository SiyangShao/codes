...
auto solve() {
  ...
  vector<int> stk(n + 1);
  auto virtualTree = [&](vector<int> &vec) {
    int tp = 1;
    stk[0] = -1, stk[1] = 0;
    G[0].clear();
    for (auto x : vec) {
      G[x].clear();
      int top = E.lca(x, stk[tp]);
      if (top == stk[tp]) {
        stk[++tp] = x;
        continue;
      }
      while (stk[tp - 1] != -1 && E.dep[stk[tp - 1]] > E.dep[top]) {
        G[stk[tp - 1]].emplace_back(stk[tp]);
        tp--;
      }
      if (top == stk[tp - 1]) {
        G[top].emplace_back(stk[tp]);
        tp--;
      } else {
        G[top].clear();
        G[top].emplace_back(stk[tp]);
  ...
  function<void(int, int)> getTree = [&](int u, int fa) {
    for (auto v : G[u]) {
      if (v == fa)
        continue;
      cout << u << " " << v << "\n";
  ...
  for (int i = 1; i < n; ++i) {
    ...
    sort(dep[i].begin(), dep[i].end(),
         [&](int x, int y) { return E.dfn[x] < E.dfn[y]; });
    virtualTree(dep[i]);
    // getTree(0, 0);
    ans += dp(0, 0, i);
    // cout << "\n";
