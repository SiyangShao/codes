#include <bits/stdc++.h>
using namespace std;
template <class Flow, class Cost> struct PD {
  struct edge {
    int u, v;
    Flow f;
    Cost c;
    edge(int _u, int _v, Flow _f, Cost _c) : u(_u), v(_v), f(_f), c(_c){};
  };
  int n;
  Cost inf_cost;
  Flow inf_flow;
  vector<vector<int>> E;
  vector<edge> edg;
  vector<pair<int, int>> p;
  vector<Cost> dis, h;
  vector<int> vis;
  PD(int _n) : n(_n), E(_n), p(_n), dis(_n), h(_n), vis(_n) {
    inf_flow = 1e9;
    inf_cost = 1e16;
  };
  void addEdge(int u, int v, Flow f, Cost c) {
    int id = (int)edg.size();
    edg.emplace_back(u, v, f, c);
    edg.emplace_back(v, u, 0, -c);
    E[u].emplace_back(id), E[v].emplace_back(id ^ 1);
  }
  bool dijkstra(int s, int t) {
    using pii = pair<Cost, int>;
    priority_queue<pii, vector<pii>, greater<>> Q;
    fill(dis.begin(), dis.end(), inf_cost);
    fill(vis.begin(), vis.end(), 0);
    dis[s] = 0;
    Q.emplace(dis[s], s);
    while (!Q.empty()) {
      int u = Q.top().second;
      Q.pop();
      if (vis[u])
        continue;
      vis[u] = 1;
      for (auto id : E[u]) {
        auto [_, v, f, c] = edg[id];
        c = c + h[u] - h[v];
        if (f > 0 && dis[v] > dis[u] + c) {
          dis[v] = dis[u] + c;
          p[v].first = u;
          p[v].second = id;
          if (!vis[v]) {
            Q.emplace(dis[v], v);
          }
        }
      }
    }
    return dis[t] != inf_cost;
  }
  void spfa(int s) {
    queue<int> Q;
    fill(h.begin(), h.end(), inf_cost);
    h[s] = 0, vis[s] = 1;
    Q.emplace(s);
    while (!Q.empty()) {
      int u = Q.front();
      Q.pop();
      vis[u] = 0;
      for (auto id : E[u]) {
        auto [_, v, f, c] = edg[id];
        if (f > 0 && h[v] > h[u] + c) {
          h[v] = h[u] + c;
          if (!vis[v]) {
            vis[v] = 1;
            Q.emplace(v);
          }
        }
      }
    }
  }
  pair<Flow, Cost> solve(int s, int t) {
    spfa(s);
    Flow maxf = 0;
    Cost minc = 0;
    while (dijkstra(s, t)) {
      auto minf = inf_flow;
      for (int i = 0; i < n; ++i) {
        h[i] += dis[i];
      }
      for (int i = t; i != s; i = p[i].first) {
        minf = min(minf, edg[p[i].second].f);
      }
      for (int i = t; i != s; i = p[i].first) {
        edg[p[i].second].f -= minf;
        edg[p[i].second ^ 1].f += minf;
      }
      maxf += minf;
      minc += minf * h[t];
    }
    return {maxf, minc};
  }
};
using ll = long long;
int main() {
  ios::sync_with_stdio(false);
  cin.tie(nullptr), cout.tie(nullptr);
  int r, c;
  cin >> r >> c;
  vector mp(r, vector<ll>(c, 0));
  for (auto &i : mp) {
    for (auto &j : i)
      cin >> j;
  }
  vector<ll> mxr(r, 0), mxc(c, 0);
  ll sum = 0, one = 0;
  ;
  for (int i = 0; i < r; ++i) {
    for (int j = 0; j < c; ++j) {
      sum += mp[i][j];
      mxr[i] = max(mxr[i], mp[i][j] - 1);
      mxc[j] = max(mxc[j], mp[i][j] - 1);
      if (mp[i][j])
        one++;
    }
  }
  ll ans = 0;
  ans += accumulate(mxr.begin(), mxr.end(), 0LL);
  ans += accumulate(mxc.begin(), mxc.end(), 0LL);
  PD<ll, ll> G(r + c + 2);
  int S = r + c, T = r + c + 1;
  for (int i = 0; i < r; ++i) {
    for (int j = 0; j < c; ++j) {
      if (mxr[i] == mxc[j] && mp[i][j] != 0) {
        G.addEdge(i, j + r, 1, -(mxr[i]));
        // cout << i << " " << j << "\n";
      }
    }
  }
  for (int i = 0; i < r; ++i) {
    G.addEdge(S, i, 1, 0);
  }
  for (int i = 0; i < c; ++i) {
    G.addEdge(i + r, T, 1, 0);
  }
  auto [flow, cost] = G.solve(S, T);
  // cout << ans << " " << flow << " " << one << " " << cost << "\n";
  ans += cost;
  // cout << ans << " " << cost << "\n";
  cout << sum - ans - one << "\n";
}