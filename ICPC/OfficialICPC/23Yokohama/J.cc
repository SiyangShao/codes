...
constexpr node inf = 1e16;
node e() { return inf; }
using F = long long;
node mapping(F f, node x) { return f + x; }
F composition(F f, F g) { return F{f + g}; } // f(g())
F id() { return F{0}; }
using lst = lazy_segtree<node, op, e, F, mapping, composition, id>;
lst seg;
struct heavy_light_decomposition {
  ...
  auto check(int u) {
    while (top[u] != 0) {
      auto mi = seg.prod(dfn[top[u]], dfn[u] + 1);
      if (mi == 0)
        return false;
      u = fa[top[u]];
    }
...
auto solve() {
  ...
  for (int i = 0; i < n; ++i) {
    seg.set(E.dfn[i], E.siz[i]);
    st.emplace(f[i], i);
  }
  function<bool(int)> canAdd = [&](int u) {
    if (!E.check(u)) {
      return false;
    }
    E.update(u);
