#include <bits/stdc++.h>
// #define ONLINE_JUDGE
#ifndef ONLINE_JUDGE
#include "dbg.h"
#else
#define dbg(...) (__VA_ARGS__)
#endif
using namespace std;
using ll = long long;
template <class Flow, class Cost> struct Primal_Dual { // based on EK
  struct edge {
    int u, v;
    Flow f;
    Cost c;
    edge(int _u, int _v, Flow _f, Cost _c) : u(_u), v(_v), f(_f), c(_c){};
  };
  int n;
  Cost inf_cost;
  Flow inf_flow;
  vector<vector<int>> E;
  vector<edge> edg;
  vector<pair<int, int>> p;
  vector<Cost> dis, h;
  vector<int> vis;
  Primal_Dual(int _n) : n(_n), E(_n), p(_n), dis(_n), h(_n), vis(_n) {
    inf_flow = 1e16, inf_cost = 1e16;
  };
  void addEdge(int u, int v, Flow f, Cost c) {
    int id = (int)edg.size();
    edg.emplace_back(u, v, f, c);
    edg.emplace_back(v, u, 0, -c);
    E[u].emplace_back(id), E[v].emplace_back(id ^ 1);
  }
  bool dijkstra(int s, int t) {
    using pii = pair<Cost, int>;
    priority_queue<pii, vector<pii>, greater<>> Q;
    fill(dis.begin(), dis.end(), inf_cost);
    fill(vis.begin(), vis.end(), 0);
    dis[s] = 0;
    Q.emplace(dis[s], s);
    while (!Q.empty()) {
      int u = Q.top().second;
      Q.pop();
      if (vis[u])
        continue;
      vis[u] = 1;
      for (auto id : E[u]) {
        auto [_, v, f, c] = edg[id];
        c = c + h[u] - h[v];
        if (f > 0 && dis[v] > dis[u] + c) {
          dis[v] = dis[u] + c;
          p[v].first = u;
          p[v].second = id;
          if (!vis[v])
            Q.emplace(dis[v], v);
        }
      }
    }
    return dis[t] != inf_cost;
  }
  void spfa(int s) {
    queue<int> Q;
    fill(h.begin(), h.end(), inf_cost);
    h[s] = 0, vis[s] = 1;
    Q.emplace(s);
    while (!Q.empty()) {
      int u = Q.front();
      Q.pop();
      vis[u] = 0;
      for (auto id : E[u]) {
        auto [_, v, f, c] = edg[id];
        if (f > 0 && h[v] > h[u] + c) {
          h[v] = h[u] + c;
          if (!vis[v])
            vis[v] = 1, Q.push(v);
        }
      }
    }
  }
  pair<Flow, Cost> solve(int s, int t) {
    spfa(s);
    Flow maxf = 0;
    Cost minc = 0;
    while (dijkstra(s, t)) {
      auto minf = inf_flow;
      for (int i = 0; i < n; ++i)
        h[i] += dis[i];
      for (int i = t; i != s; i = p[i].first)
        minf = min(minf, edg[p[i].second].f);
      for (int i = t; i != s; i = p[i].first)
        edg[p[i].second].f -= minf, edg[p[i].second ^ 1].f += minf;
      maxf += minf, minc += minf * h[t];
    }
    return {maxf, minc};
  }
};
auto solve() {
  int n, m;
  cin >> n >> m;
  vector<vector<int>> mp(n, vector<int>(m));
  for (auto &i : mp) {
    for (auto &j : i)
      cin >> j;
  }
  Primal_Dual<ll, ll> G(n * m * 2 + 5);
  int S = n * m * 2, T = n * m * 2 + 1, mid = n * m * 2 + 2;
  int ss = mid + 1, tt = mid + 2;
  auto in = [&](int i, int j) { return i * m + j; };
  auto out = [&](int i, int j) { return i * m + j + n * m; };
  G.addEdge(S, mid, 1, 0);
  ll cnt = 0;
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < m; ++j) {
      G.addEdge(in(i, j), out(i, j), 1, mp[i][j]);
      cnt += mp[i][j];
    }
  }
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < m; ++j) {
      G.addEdge(in(i, j), tt, 1, 0);
      G.addEdge(ss, out(i, j), 1, 0);
    }
  }
  for (int i = 0; i < 1; ++i) {
    for (int j = 0; j < m; ++j) {
      G.addEdge(mid, in(i, j), 1, 0);
    }
  }
  for (int i = 0; i + 1 < n; ++i) {
    for (int j = 0; j < m; ++j) {
      G.addEdge(out(i, j), in(i + 1, j), 1, 0);
      G.addEdge(out(1 + i, j), in(i, j), 1, 0);
    }
    for (int j = 0; j + 1 < m; ++j) {
      G.addEdge(out(i, j), in(i, j + 1), 1, 0);
      G.addEdge(out(i, j + 1), in(i, j), 1, 0);
    }
  }
  for (int i = n - 1; i < n; ++i) {
    for (int j = 0; j < m; ++j) {
      G.addEdge(out(i, j), T, 1, 0);
    }
  }
  auto [_, base] = G.solve(ss, tt);
  G.n -= 2;
  auto [mx, mi] = G.solve(S, T);
  cout << mi + base + cnt << "\n";
}
auto main() -> int {
  ios::sync_with_stdio(false);
  cin.tie(nullptr), cout.tie(nullptr);
  int _ = 1;
  // cin >> _;
  while (_--) {
    solve();
  }
}