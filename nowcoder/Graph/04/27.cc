...
int main() {
  ...
  int n, m;
  cin >> m >> n;
  Primal_Dual<long long, long long> G(n + m + 2), nG(n + m + 2);
  int S = n + m, T = n + m + 1;
  for (int i = 0; i < m; ++i) {
    int a;
    cin >> a;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    cin >> b;
    G.addEdge(i + m, T, b, 0);
    nG.addEdge(i + m, T, b, 0);
  }
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      int w;
      cin >> w;
      G.addEdge(i, j + m, numeric_limits<long long>::max(), w);
      nG.addEdge(i, j + m, numeric_limits<long long>::max(), -w);
    }
  }
  auto [maxf, minc] = G.solve(S, T);
  auto [_, maxc] = nG.solve(S, T);
  cout << minc << "\n";
  cout << -maxc << "\n";
}