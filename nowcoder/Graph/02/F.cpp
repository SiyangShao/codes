...
auto solve() {
    ...
    int n, m;
    cin >> n >> m;
    vector<pair<int, int>> del(m);
    auto cal = [&](int i, int j) { return (i - 1) * n + j; };
    set<int> st;
    for (auto &[x, y] : del) {
        cin >> x >> y;
        st.insert(cal(x, y));
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= n; ++j) {
            if (st.find(cal(i, j)) != st.end())
                continue;
            if ((i + j) % 2 == 0) {
                g.add_edge(0, cal(i, j), 1);
                if (i + 1 <= n && st.find(cal(i + 1, j)) == st.end()) {
                    g.add_edge(cal(i, j), cal(i + 1, j), 1);
                }
                if (i - 1 >= 1 && st.find(cal(i - 1, j)) == st.end()) {
                    g.add_edge(cal(i, j), cal(i - 1, j), 1);
                }
                if (j - 1 >= 1 && st.find(cal(i, j - 1)) == st.end()) {
                    g.add_edge(cal(i, j), cal(i, j - 1), 1);
                }
                if (j + 1 <= n && st.find(cal(i, j + 1)) == st.end()) {
                    g.add_edge(cal(i, j), cal(i, j + 1), 1);
                }
            } else {
                g.add_edge(cal(i, j), n * n + 1, 1);
            }
        }
    }
    cout << g.flow(0, n * n + 1) << "\n";
